...
export default class AboutMe extends Component {
  componentWillMount() {
    getClient().getEntries({content_type: 'skillTable'}) // eslint-disable-line camelcase
      .then(entries => {
        this.setState({
  ...
  render(props, state) {
    return (
      <Page class="AboutMe" title="About Me">
            ...
            {state.skills.map(skillTable => (
              <SkillTable key={skillTable.title} title={skillTable.title}>
                {skillTable.skills && skillTable.skills.map(skill => (
                  <SkillTableItem key={skill.name} icon={skill.image}>{skill.name}</SkillTableItem>
                ))}
              </SkillTable>
            ))}
          </div>
        ...
        <div class="AboutMe-content">
          <p>And I also have experience on personal projects using these tools, and I'm quite excited about them (more info on my <Link to="/portfolio">portfolio</Link>).</p>
